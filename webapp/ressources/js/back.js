$(document)
		.ready(
				function() {
					...
					// ARTISTS ---
					$('.existingArtistList .addable')
							.click(
									function() {
										e = $(this);
										artistToAdd = $(e).html();
										artistBox = e.closest(".artists")
												.prev().children(".box");
										artistBoxHtml = artistBox.html();
										if (isNotYetMentionned(artistBox,
												artistToAdd)
												&& artistToAdd != '') {
											artistToAdd = '<span title="Supprimer" class="bubble removable">'
													+ artistToAdd + '</span>';
											artistBox.html(artistBoxHtml
													+ artistToAdd);
										}
									});
					...
					$('.box.artistList:not(".box.artistList.bubble")')
							.click(
									function() {
										e = $(this);
										artistBox = $(this);
					...
					$('.box')
							.on(
									'keypress',
									'.new_artist, .new_genre, .new_album',
									function(c) {
										e = $(this);
										code = (c.keyCode ? c.keyCode : c.which);
										separator1 = ','.charCodeAt(0);
										separator2 = ';'.charCodeAt(0);
										if (code == separator1
												|| code == separator2) {
											artistToAdd = e.val();
					...
					$('.box').on('blur', '.new_artist, .new_genre, .new_album',
							function() {
								// $(this).remove();
							});
					...
					// genreToAdd = '<span title="Supprimer" class="bubble
					// removable">'+genreToAdd+'</span>';
					...
					// if(!previousToAppend.length) {
					// toAppend = '<input title="Supprimer" type="text"
					// class="new_genre"/>';
					...
					// ALBUMS ---
					$('.existingAlbumList .addable')
							.click(
									function() {
										e = $(this);
										albumToAdd = $(e).html();
										albumBox = e.closest(".albums").prev()
												.children(".box");
										albumBoxHtml = albumBox.html();
										if (isNotYetMentionned(albumBox,
												albumToAdd)
												&& albumToAdd != ''
												&& albumBoxHtml <= 0) {
											albumToAdd = '<span title="Supprimer" class="bubble removable">'
													+ albumToAdd + '</span>';
											albumBox.html(albumBoxHtml
													+ albumToAdd);
										}
									});
					...
					$('.box.albumList:not(".box.albumList.bubble")')
							.click(
									function() {
										e = $(this);
										albumBox = $(this);
					...
					$('.line.dropable').click(function() {
						e = $(this);
						infosBox = e.nextAll('.line.infos').first();
					...
					$("#delete").click(
							function(e) {
								url = "/MuzikKloud/admin/removesong?id="
										+ parseInt($("#SongToRemove").val());
								e = $(this);
								window.location = url;
							});
					...
					$("td")
							.on(
									...
									function() {
										e = $(this);
										ex_name = e.text();
										id = e.closest(".song").find(
												".idArtist").val();
										td = e.closest("td");
										e.addClass("hidden");
										exHtml = td.html();
										toAppend = '<input type="text" value="'
												+ ex_name
												+ '" class="new_artist"/><button class="ok">Ok</button><input type="hidden" class="exName" value="'
												+ ex_name
					...
					$("td")
							.on(
									...
									function() {
										...
										id = e.closest(".song").find(
												".idArtist").val();
										type = e.closest(".song").find(".type")
												.val();
										alert("/MuzikKloud/admin/" + type
												+ "?id=" + id + "&name="
												+ new_value);
										$.ajax({
											type : "POST",
											url : "/MuzikKloud/admin/" + type
													+ "?id=" + id + "&name="
													+ new_value,
					...
					$("td").on(
							...
							".cancel",
							function() {
								$(this).closest("td").find(".nameArtist")
										.removeClass("hidden");
								$(this).closest("td").find(".ok").remove();
								$(this).closest("td").find(".new_artist")
										.remove();
